...
export const JobDetail: React.FC<JobDetailProps> = ({
  ...
  // Raw PCM decoding helpers (unchanged)
  ...
  const handleGenerateDocuments = async () => {
    if (!userProfile.resumeContent || userProfile.resumeContent.length < 50) {
      notify(
        "Please update your master resume in Settings first.",
        "error"
      );
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-y-auto pb-24">
      <div className="p-8 max-w-5xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-indigo-900 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
            ...
            <p className="text-sm font-medium leading-relaxed mb-6 opacity-80 italic pe-8">
              "Hi {userProfile.fullName}, I've analyzed this role. It matches
              your background perfectly. Let's listen to the key points..."
            </p>
            <button
            ...
            <div className="space-y-3">
              {questions.length > 0 ? (
                questions.map((q, i) => (
                  ...
                  >
                    <span className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400 shrink-0">
                      {i + 1}
                    </span>
                ...
                <p className="text-xs text-slate-400 italic">
                  Generate mock questions based on your profile for this
                  specific role.
                </p>
              )}
            </div>
        ...
        {/* Description + AI Assets */}
        <div className="grid grid-cols-1 gap-8">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-8 flex items-center gap-2">
              <ChevronDown className="w-4 h-4 text-indigo-600" /> Full
              Description
            </h3>
